...
def get_primes(nmax, efficientcy=True):
    if efficientcy == True:
        return prime_efficient(nmax)
    else:
        return prime(nmax);

    

def proportion_primes(low,high, gap):
    """
    proportion of prime numbers smaller than some nmax
    """
    all_nmax = np.arange(low, high, gap)
    all_proportions = []
